...
class enrol_stripe_testcase extends advanced_testcase {
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['stripe']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('stripe'));
        $plugin = enrol_get_plugin('stripe');
        $this->assertInstanceOf('enrol_stripe_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_stripe', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $stripeplugin = enrol_get_plugin('stripe');

        // Just make sure the sync does not throw any errors when nothing to do.
        $stripeplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_stripe_plugin $stripeplugin  */
        $stripeplugin = enrol_get_plugin('stripe');
        ...
        $stripeplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $stripeplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $stripeplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $stripeplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $stripeplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $stripeplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $stripeplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $stripeplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $stripeplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $stripeplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $stripeplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $stripeplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $stripeplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
